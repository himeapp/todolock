...
enum LegalDoc {
    ...
    static let effectiveDate = "2026-09-04"
    ...
    static let privacy: [Section] = [
        ...
        • 사진 인증 과제로 촬영·선택한 사진은 기기 안에서만 검수되며 외부로 전송·저장되지 않습니다.
        ...
        애플리케이션은 어떠한 개인정보도 수집하지 않습니다. 별도의 서버가 없으며 네트워크 통신을 하지 않으므로, 귀하의 정보가 기기 밖으로 나가지 않습니다.

        • IP 주소·기기 식별자·광고 식별자를 수집하지 않습니다.
        • 위치 정보를 수집하지 않습니다.
        • 이름·이메일 등 연락처 정보를 수집하지 않습니다.
        • 이용 기록이나 분석·통계 데이터를 수집하지 않습니다.

        분석 도구, 광고 SDK, 제3자 추적 기술을 포함하지 않으며, 귀하의 데이터를 처리하기 위해 인공지능(AI) 기술을 사용하지 않습니다.
        """),
        .init(heading: "제3자 제공", body: """
        제3자에게 제공하는 정보가 없습니다. 수집하는 정보 자체가 없으므로 판매·공유·이전할 정보도 존재하지 않습니다.
        """),
        .init(heading: "삭제", body: """
        기기에 저장된 데이터는 귀하가 직접 삭제하거나 애플리케이션을 삭제할 때까지 기기 안에 남습니다. 애플리케이션을 삭제하면 관련 데이터가 기기에서 함께 삭제되며, 서비스 제공자가 따로 보관하는 사본은 없습니다.
        """),
        ...
        서비스 제공자가 보관하는 귀하의 정보가 없으므로 유출될 서버 측 정보도 없습니다. 기기 안의 데이터는 iOS가 제공하는 앱 샌드박스와 기기 암호화로 보호됩니다.
        """),
